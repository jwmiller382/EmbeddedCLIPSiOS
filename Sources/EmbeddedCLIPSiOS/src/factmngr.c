   ...
   if (theFact->whichDeftemplate->named)
     {
      ...
      AddNamedFactToHashMap(theEnv,theFact,theName);
     }
...
#if CERTAINTY_FACTORS
   hashValue = HandleFactDuplication(theEnv,theFact,&duplicate,reuseIndex,(short) cf);
#else
   hashValue = HandleFactDuplication(theEnv,theFact,&duplicate,reuseIndex,0);
#endif
   if (duplicate != NULL)
     { return duplicate; }
